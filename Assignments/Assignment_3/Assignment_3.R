...
head(letters,5) # First 5
tail(letters,5) # Last 5
letters[15] # Just the 15th letter
...
class(vector3) # What is the class/kind of vector?
class(vector1)
class(vec)
...
letters %in% c("a","b","c","z") # %n% = is the thing on the left (letters) found in the thing on the right ("a","b","c","z")
vector1 %in% 1:6
...
# convert to a factor
as.factor(nums) # show in console --- "categorized" the numbers
...
big_area_iris <- subset(iris, rowSums(iris[6]>20)>0)
big_area_iris
